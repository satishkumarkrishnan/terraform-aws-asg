...
export efs_name=${efs_hostname}
echo ${efs_name} >> /tmp/efs/efs_name.txt
systemctl enable --now nfs-server
# Actually mount the EFS filesystem
sudo mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,soft,timeo=600,retrans=2,noresvport {efs_name}:/ /tmp/efs
# Mounting Efs 
